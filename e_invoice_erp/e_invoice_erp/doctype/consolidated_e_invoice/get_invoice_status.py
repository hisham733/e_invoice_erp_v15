...
def get_lhdn_submission_status(doc):
    ...
    try:
        ...
        sales_e_invoice_doc = frappe.get_doc("Consolidated E Invoice", doc.get("name"))
        sales_e_invoice_doc.custom_lhdn_e_invoice_status = status
        custom_lhdn_status = status
        invoice_name = sales_e_invoice_doc.name

        for row in sales_e_invoice_doc.invoices:
            if row.original_invoice:
                ...
                frappe.db.set_value(
                    ...
                    {
                        "custom_consolidate_invoice_number": invoice_name,
      ...
      #   sales_e_invoice_doc.lhdn_validation_result = validation_result_text
        ...
        if status == 'Invalid':
            frappe.msgprint(
                _("LHDN Status: <b>{0}</b>. Please check the 'LHDN Validation Result' field for details.").format(status),
                title=_("Status Update"), 
                indicator="green" # Use orange for warnings
            )
        else:
            frappe.msgprint(
                _("LHDN submission status updated successfully: <b>{0}</b>").format(status),
                indicator="green"
            )
